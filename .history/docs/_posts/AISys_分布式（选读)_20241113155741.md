...
在Tensor模型并行中只在Attention和MLP两个地方进行了并行计算，对于两块的输入并没有并行操作。    
图中 f f f 和 f ‾ \overline{f} f​ 互为共轭(conjugate)， f f f 在前向时不做操作，反向时执行all-reduce; f ‾ \overline{f} f​ 在前向时执行all-reduce, 反向时不做操作。
