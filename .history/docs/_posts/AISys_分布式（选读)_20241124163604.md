...
对$X$按sequence维度切分， $X = [ X_1^s , X 2 s ] X = \left[ X^s_1, X^s_2 \right] X=[X1s​,X2s​]，LayerNorm的结果 Y = [ Y 1 s , Y 2 s ] Y = \left[ Y^s_1, Y^s_2 \right] Y=[Y1s​,Y2s​]；
由于接下来的GeLU不是线性的，所以要进行all-gather操作，计算 Z = G e L U ( Y A ) Z = GeLU(YA) Z=GeLU(YA)；
对 A A A 进行列切分的tensor并行，得到结果 Y A 1 c YA^c_1 YA1c​ 和 Y A 2 c YA^c_2 YA2c​
对 B B B 进行行切分的tensor并行，得到结果 Z 1 h B 1 r Z^h_1 B^r_1 Z1h​B1r​ 和 Z 2 h B 2 r Z^h_2 B^r_2 Z2h​B2r​
得到 W 1 W_1 W1​ 和 W 2 W_2 W2​ 后进行累加操作(reduce-scatter)